...
- [x] ADR on deferring `shadcn/ui`. Covers the two-token-vocabulary cost, the fact that none of V1's components need Radix's accessibility machinery, and the trigger for reaching for it: the first component that genuinely does.
- [x] Each ADR uses the glossary's vocabulary and avoids the terms marked to avoid.
- [x] ADRs are numbered and live at the repo root's decision record directory, per the single-context layout.
